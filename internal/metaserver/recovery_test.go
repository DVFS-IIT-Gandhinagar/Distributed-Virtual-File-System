...
func TestNavigateFailsGracefullyWithoutAnyFileServer(t *testing.T) {
	...
	h := NewGRPCHandler(ms)
	resp, err := h.Navigate(context.Background(), &pb.NavigateRequest{User: "alice"})
	if err != nil {
...
func TestNavigateRemapsFromStaleToHealthyServer(t *testing.T) {
	stateFile := filepath.Join(t.TempDir(), "metaserver_state.json")
	...
	resp2, err := h.RegisterFileServer(context.Background(), &pb.RegisterFileServerRequest{
	...
	if !resp2.Success {
		t.Fatalf("second register failed: %s", resp2.Error)
	}
	...
	navResp, err := h.Navigate(context.Background(), &pb.NavigateRequest{User: "alice"})
	if err != nil {
	...
	if navResp.Address != "127.0.0.1:7102" {
		t.Fatalf("expected remap to healthy fs 127.0.0.1:7102, got %s", navResp.Address)
	}
}
